...
::  要激活，请运行脚本并使用 "/act" 参数或将下一行中的 0 更改为 1
set _activate=0

::  要冻结 30 天试用期，请运行脚本并使用 "/frz" 参数或将下一行中的 0 更改为 1
set _freeze=0

::  要重置激活和试用，请运行脚本并使用 "/res" 参数或将下一行中的 0 更改为 1
set _reset=0
...
:: 如果脚本由 x86 进程在 x64 位 Windows 上启动，则使用 x64 进程重新启动脚本
:: 或者，如果脚本由 x86/ARM32 进程在 ARM64 Windows 上启动，则使用 ARM64 进程重新启动脚本
...
:: 如果脚本由 x64 进程在 ARM64 Windows 上启动，则使用 ARM32 进程重新启动脚本
...
%eline%
echo 脚本是从临时文件夹启动的，
echo 很可能您直接从存档文件运行脚本。
echo:
echo 请提取存档文件并从提取的文件夹中启动脚本。
goto done2
)
)
...
echo:
echo PowerShell 无法工作。中止...
echo 如果您已对 Powershell 应用了限制，请撤销这些更改。
echo:
...
::  以管理员身份提升脚本并传递参数并防止循环

%nul1% fltmc || (
if not defined _elev %psc% "start cmd.exe -arg '/c \"!_PSarg!\"' -verb runas" && exit /b
%eline%
echo 此脚本需要管理员权限。
echo 要这样做，请右键单击此脚本并选择 "以管理员身份运行"。
goto done2
)

::========================================================================================================================================

::  禁用快速编辑并从 conhost.exe 启动以避免使用终端应用
...
::  以下代码也适用于 ARM64 Windows 10（包括 x64 位模拟）
...
echo:            ___________________________________________________
echo:         
call :_color2 %_White% "             " %_Green% "在键盘上输入菜单选项 [1,2,3,4,5,0]"
choice /C:123450 /N
set _erl=%errorlevel%
...
echo:
echo 正在创建 CLSID 注册表键的备份到 %SystemRoot%\Temp
...
echo %line%
echo:
echo      有些用户激活不起作用，IDM 可能会显示假冒序列号的纳屏。
echo:
...
:: 使用 internetdownloadmanager.com ping 和端口 80 测试连接到互联网
...
call :_color %Red% "无法连接到 internetdownloadmanager.com，中止..."
goto done
)
call :_color %Gray% "Ping 命令失败于 internetdownloadmanager.com"
echo:
)
...
echo:
call :_color %Gray% "如果出现假冒序列号的纳屏，请使用冻结试用选项。"
) else (
call :_color %Green% "IDM 30 天试用期已成功冻结为终身。"
echo:
call :_color %Gray% "如果 IDM 显示弹出窗口要求注册，请重新安装 IDM。"
)
