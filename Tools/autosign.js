...
var src_default = {
    ...
    async scheduled(event, env, ctx) {
        ctx.waitUntil(executeTask(env));
    }
};
async function executeTask(env) {
    const client = buildLibsqlClient(env);
    try {
        console.log("\u4EFB\u52A1\u5F00\u59CB: \u53D1\u9001POST\u8BF7\u6C42");
        const url = "https://www.idc.net/addons?_plugin=13&_controller=index&_action=index";
        const headers = {
            ...
            "cookie": "很长一坨",
            "Referer": "https://www.idc.net/addons?_plugin=13&_controller=index&_action=index",
            "Referrer-Policy": "strict-origin-when-cross-origin"
        };
        const body = new URLSearchParams({ "uid": "咳咳" });
        ...
        const resultText = JSON.parse(result).msg;
        console.log("\u54CD\u5E94\u72B6\u6001:", response.status);
        console.log("\u54CD\u5E94\u5185\u5BB9:", resultText);
        await logToDatabase(client, `\u4EFB\u52A1\u6210\u529F: ${resultText}`);
    } catch (error) {
        console.error("\u8BF7\u6C42\u5931\u8D25:", error);
        await logToDatabase(client, `\u8BF7\u6C42\u5931\u8D25: ${error.message}`);
    }
}
...
async function logToDatabase(client, message) {
    const timestamp = (/* @__PURE__ */ new Date()).toISOString();
    const datetime = (/* @__PURE__ */ new Date()).toLocaleString("en-US", { timeZone: "UTC", hour12: false });
    try {
        await client.execute(
            "INSERT INTO logs (timestamp, datetime, message) VALUES (?, ?, ?)",
            [timestamp, datetime, message]
        );
        console.log("\u65E5\u5FD7\u8BB0\u5F55\u6210\u529F:", message);
    } catch (dbError) {
